...
export class Amazon extends Site implements IAmazonConfig {
	...
	public constructor(amazonConfig: IAmazonConfig, completeTransactions: boolean) {
		...
		this.reloadDelayInSeconds = typeof amazonConfig.reloadDelayInSeconds === 'undefined' ? 300 : amazonConfig.reloadDelayInSeconds;
	}
	...
	private async addCard(card: Card): Promise<void> {
		...
		await driver.findElement(By.css('.a-size-base')).click();

		if (card.cardholderName !== undefined && card.expirationMonth !== undefined && card.expirationYear !== undefined) {
			await driver.findElement(By.name('ppw-accountHolderName')).sendKeys(card.cardholderName);
			await driver.findElement(By.name('addCreditCardNumber')).sendKeys(card.cardNumber);
			await driver.findElement(By.xpath(`//select[@name='ppw-expirationDate_month']/option[${card.expirationMonth}]`)).click();
			await driver.findElement(By.xpath(`//select[@name='ppw-expirationDate_year']/option[.=${card.expirationYear}]`)).click();
			await driver.findElement(By.name('ppw-widgetEvent:AddCreditCardEvent')).click();

			await driver.wait(until.elementLocated(By.name('ppw-widgetEvent:SelectAddressEvent'))).click();
		} else {
	...
	private async login(): Promise<void> {
		...
		try {
			...
			assert((await driver.findElement(By.css('.a-size-base')).getText()) === this.username);
		}
	...
	private async reloadCard(card: Card): Promise<void> {
		...
		if (this.completeTransactions) {
			...
			const confirmationButtons: WebElement[] = await driver.findElements(By.xpath("//button[contains(.,'Verify card')]"));
